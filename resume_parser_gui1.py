...
jd_option = st.radio("JD Input Type", ("Text", "File (CSV/PDF/DOCX)"))  # Updated label to include DOCX

if jd_option == "Text":
    job_desc = st.text_area("Paste JD Text Here", height=200)
else:
    st.caption("Supported formats: CSV, PDF, DOCX")
    jd_file = st.file_uploader("Upload JD File (CSV, PDF or DOCX)", type=['csv', 'pdf', 'docx', 'DOCX'])
    if jd_file:
